...
// Assuming text is defined and contains an array of lines

const convertLinesToJson = (lines) => {
  const jsonObjects = [];

  // Iterate through each line
  lines.forEach((line) => {
    // Convert the line to a JSON object
    const jsonObject = lineToJson(line);
    if (jsonObject) {
      // If a valid JSON object is returned, push it to the array
...
const lineToJson = (line) => {
  // Match the packType, source, destination, rewardType, and rewardValue using regular expressions
  const matches = line.match(/(\w+)\["([^"]+)"\]\["([^"]+)"\]\.(\w+)\s*=\s*([\d.]+);/);
  if (!matches) return null;

  // Extract the matched groups
  const [, packType, source, destination, rewardType, rewardValue] = matches;
  ...
  return {
    ...
    destination,
    rewardType,
    rewardValue: parseFloat(rewardValue) // Parse the reward value as float to handle decimals
  };
};


// Convert lines to JSON objects
const jsonObjects = convertLinesToJson(text);

// Write JSON objects to output.txt
fs.writeFile('output.txt', JSON.stringify(jsonObjects, null, 2), (err) => {
  if (err) {
